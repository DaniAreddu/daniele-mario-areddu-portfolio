...
import { useAdminProjectList } from "@/admin/hooks/useAdminProjects";
import type { AdminProjectListItem } from "@/admin/types/project";

function publicationTone(status: AdminProjectListItem["publication_status"]) {
  switch (status) {
    case "PUBLISHED":
...
export default function ProjectsListPage() {
  ...
  const { data: projects, isLoading, isError } = useAdminProjectList(showTrashed);

  const filtered = useMemo(() => {
    if (!projects) return [];
    const query = search.trim().toLowerCase();
    if (!query) return projects;
    return projects.filter((project) => project.title_en.toLowerCase().includes(query));
  }, [projects, search]);
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 font-serif text-3xl text-ink">Case studies</h1>
        </div>
        <LinkButton to="/admin/projects/new">New project</LinkButton>
      </div>
      ...
      {isLoading ? (
        ...
        <p className="mt-8 text-sm text-red-700">Could not load projects.</p>
      ) : filtered.length === 0 ? (
        <div className="mt-8 rounded-2xl border border-dashed border-ink/15 p-10 text-center">
          <p className="text-ink-soft">
            {showTrashed ? "Trash is empty." : "No projects yet."}
          </p>
        </div>
      ) : (
              ...
              {filtered.map((project) => (
                <tr key={project.id} className="border-b border-ink/5 last:border-0">
                  ...
                  <td className="px-4 py-3 text-ink-soft">
                    {project.is_featured ? "Yes" : "—"}
                  </td>
                  <td className="px-4 py-3">
                    <Badge tone={publicationTone(project.publication_status)}>
