...
import {
  useAdminEducationItem,
  useAdminEducationRevisions,
  useArchiveEducation,
  useCreateEducation,
  usePermanentlyDeleteEducation,
  useRestoreEducation,
  useRestoreEducationRevision,
  useScheduleEducation,
  useTrashEducation,
  useUnpublishEducation,
  useUpdateEducation,
} from "@/admin/hooks/useAdminEducation";
import {
  defaultEducationFormValues,
  educationFormSchema,
  educationFormValuesToPayload,
  educationToFormValues,
  type EducationFormValues,
} from "@/admin/pages/education/educationFormSchema";
import type { EducationAdminDetail } from "@/admin/types/education";
...
export default function EducationEditorPage() {
  ...
  const { data: item, isLoading } = useAdminEducationItem(itemId);
  const { data: revisions } = useAdminEducationRevisions(itemId);
  ...
  useEffect(() => {
    if (item) form.reset(educationToFormValues(item));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [item]);

  const createMutation = useCreateEducation();
  const updateMutation = useUpdateEducation(itemId ?? -1);
  const scheduleMutation = useScheduleEducation(itemId ?? -1);
  const unpublishMutation = useUnpublishEducation(itemId ?? -1);
  const archiveMutation = useArchiveEducation(itemId ?? -1);
  const trashMutation = useTrashEducation(itemId ?? -1);
  const restoreMutation = useRestoreEducation(itemId ?? -1);
  const deleteMutation = usePermanentlyDeleteEducation();
  const restoreRevisionMutation = useRestoreEducationRevision(itemId ?? -1);

  async function persist(values: EducationFormValues): Promise<EducationAdminDetail> {
    const payload = educationFormValuesToPayload(values);
    if (isNew) {
      const created = await createMutation.mutateAsync(payload);
      navigate(`/admin/education/${created.id}`, { replace: true });
      return created;
    }
  ...
  const onPublish = form.handleSubmit(async (values) => {
    ...
    try {
      ...
      await adminApiFetch(`/education/${saved.id}/publish`, { method: "POST" });
      await queryClient.invalidateQueries({ queryKey: ["admin", "education"] });
      showToast({ title: "Published" });
      navigate(`/admin/education/${saved.id}`, { replace: true });
    } catch (error) {
      setServerError(describeError(error));
    }
  });
    ...
    <div className="max-w-3xl">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 font-serif text-3xl text-ink">
            {isNew ? "New entry" : form.watch("institution") || "Edit entry"}
          </h1>
        </div>
      ...
      <form className="mt-6 space-y-8" onSubmit={(event) => event.preventDefault()}>
        <Card className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Field
              ...
              htmlFor="degree_en"
              error={form.formState.errors.degree_en?.message}
            >
              <Input id="degree_en" {...form.register("degree_en")} />
            </Field>
            <Field label="Degree (Italian)" htmlFor="degree_it">
              <Input id="degree_it" {...form.register("degree_it")} />
            </Field>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            >
              <Input id="start_year" inputMode="numeric" {...form.register("start_year")} />
            </Field>
            <Field
              label="End year"
              htmlFor="end_year"
            ...
            >
              <Input id="end_year" inputMode="numeric" {...form.register("end_year")} />
            </Field>
          ...
          <Field label="Activities (one per line)" htmlFor="activities_input">
            <Textarea id="activities_input" rows={3} {...form.register("activities_input")} />
          </Field>
        </Card>
        ...
        <div className="flex flex-wrap items-center gap-3 border-t border-ink/10 pt-6">
          <LinkButton to="/admin/education" variant="secondary">
            Back to list
          </LinkButton>
          {!isNew && item ? (
            <LinkButton to={`/admin/education/${item.id}/preview`} variant="secondary">
              Preview
            </LinkButton>
          ) : null}
          <Button
