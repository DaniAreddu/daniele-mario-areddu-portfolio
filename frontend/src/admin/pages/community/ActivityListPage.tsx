...
import { useAdminActivityList } from "@/admin/hooks/useAdminCommunity";
import type { CommunityActivityListItem } from "@/admin/types/community";

function publicationTone(status: CommunityActivityListItem["publication_status"]) {
  switch (status) {
    case "PUBLISHED":
...
export default function ActivityListPage() {
  ...
  const { data: items, isLoading, isError } = useAdminActivityList(showTrashed);
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 font-serif text-3xl text-ink">Activities</h1>
        </div>
        <div className="flex items-center gap-3">
          <LinkButton to="/admin/community/profile" variant="secondary">
            Edit profile
          </LinkButton>
          <LinkButton to="/admin/community/new">New activity</LinkButton>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <p className="mt-8 text-sm text-red-700">Could not load activities.</p>
      ) : filtered.length === 0 ? (
        <div className="mt-8 rounded-2xl border border-dashed border-ink/15 p-10 text-center">
          <p className="text-ink-soft">
            {showTrashed ? "Trash is empty." : "No activities yet."}
          </p>
        </div>
      ) : (
              ...
              {filtered.map((item) => (
                <tr key={item.id} className="border-b border-ink/5 last:border-0">
                  <td className="px-4 py-3">
                    <Link
                      to={`/admin/community/${item.id}`}
                      className="font-medium text-ink hover:text-cobalt"
                    >
                      {item.title_en}
                    </Link>
                  </td>
                  <td className="px-4 py-3 text-ink-soft">{item.activity_type}</td>
                  <td className="px-4 py-3">
